...
### 沈黙保持要素：61件
構造・入力・ボタン・セグメント・カード・バッジ・スコア・信頼度・地図・空状態

### 沈黙破壊リスク要素：5件（記録のみ）
| 要素 | リスク要因 | 備考 |
|------|-----------|------|
| body | background gradient | 空間定義（情報ではない） |
| .mode-section.hidden | opacity:0 | 構造的非表示 |
| .station-name | font-size:20px | 識別目的 |
| .badge-dev | #FF2D55 | 開発モード専用 |
| .legend-note | font-size:10px | 補足的位置 |
...
| 要素 | 初期 | 遷移 | 差分 | 判定 |
|------|------|------|------|------|
| .seg-item.active | 0 | 0 | 0 | 維持 |
| .sort-btn.active | 0 | 0 | 0 | 維持 |
| .mode-section.visible | 0 | 0 | 0 | 維持 |
| #map.visible | 0 | 0 | 0 | 維持 |
